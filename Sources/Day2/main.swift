...
func checkEntry(entry: PasswordEntry) -> Bool {
    let count = entry.password.count(of: entry.letter)
    return count <= entry.maximum && count >= entry.minimum
}

func checkEntries(entry: [PasswordEntry]) -> [(PasswordEntry, Bool)] {
    entries.map {
        ($0, checkEntry(entry: $0))
    }
}
...
let checkedEntries = checkEntries(entry: entries)
let failedEntries = getFailedEntries(entries: checkedEntries)
print(failedEntries)

let validEntries = entries.count - failedEntries.count
print(validEntries)
